...
DATA_FILE = "pppub19.csv"           # 2019 ASEC, tax year 2018
FILING_STATUS_MAX = 4               # FILESTAT < 4 for married filing jointly

STD_DEDUCTION = 24000                # 2018 MFJ standard deduction
BRACKETS = np.array([19050, 77400, 165000, 315000, 400000, 600000])
...
G_GRID = np.arange(0.115, 0.145, 0.001)
SIGMA_GRID = np.arange(1.05, 1.140, 0.001)
...
# Preferential tax (2018 MFJ thresholds: 0% up to $77,200; 15% up to $479,000)
def calculate_preferential_tax(pref_income, agi):
    tax = np.zeros_like(pref_income)
    mask_15 = (agi > 77200) & (agi <= 479000)
    mask_20 = agi > 479000
