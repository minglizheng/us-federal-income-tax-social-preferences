...
DATA_FILE = "pppub20.csv"           # 2020 ASEC, tax year 2019
FILING_STATUS_MAX = 4               # FILESTAT < 4 for married filing jointly

STD_DEDUCTION = 24400                # 2019 MFJ standard deduction
BRACKETS = np.array([19400, 78950, 168400, 321450, 408200, 612350])
...
G_GRID = np.arange(0.130, 0.150, 0.001)
SIGMA_GRID = np.arange(1.050, 1.130, 0.001)
...
# Preferential tax (2019 MFJ thresholds: 0% up to $78,750; 15% up to $488,850)
def calculate_preferential_tax(pref_income, agi):
    tax = np.zeros_like(pref_income)
    mask_15 = (agi > 78750) & (agi <= 488850)
    mask_20 = agi > 488850
